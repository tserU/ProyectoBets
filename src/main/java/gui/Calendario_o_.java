...
public class Calendario_o_ extends JFrame {
	...
	/// Yo
	private Vector<Event> v = new Vector<Event>();

	private Vector<Date> datesWithEventsCurrentMonth = new Vector<Date>();

	public Calendario_o_() {
		try {
			jbInit(v);
		} catch (Exception e) {
			e.printStackTrace();
...
//					public void propertyChange(PropertyChangeEvent propertychangeevent) {
				if (propertychangeevent.getPropertyName().equals("locale")) {
					...
					/*
					 * Vector<domain.Event> events = facade.getEvents(firstDay); if
					 * (events.isEmpty())
					 * jLabelListOfEvents.setText(ResourceBundle.getBundle("Etiquetas").getString(
					 * "NoEvents") + ": " + dateformat1.format(calendarAct.getTime())); else
					 * jLabelListOfEvents.setText(ResourceBundle.getBundle("Etiquetas").getString(
					 * "Events") + ": " + dateformat1.format(calendarAct.getTime()));
					 * jComboBoxEvents.removeAllItems(); System.out.println("Events " + events);
					 * 
					 * for (domain.Event ev : events) modelEvents.addElement(ev);
