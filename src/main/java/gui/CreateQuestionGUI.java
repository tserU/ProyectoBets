...
public class CreateQuestionGUI extends JFrame {
	...
	private JTextField apuestaMinima = new JTextField();
	...
	private void jbInit(Vector<domain.Event> v) throws Exception {
		...
		apuestaMinima.setBounds(new Rectangle(100, 243, 60, 20));
		...
		this.getContentPane().add(apuestaMinima, null);
	...
	private void jButtonCreate_actionPerformed(ActionEvent e) {
		...
		try {
			...
			if (inputQuery.length() > 0) {
				...
				float valorApuestMin = Float.parseFloat(apuestaMinima.getText());
				...
				if (valorApuestMin <= 0)
					...
					facade.createQuestion(event, inputQuery, valorApuestMin);
